	...
	public void explosionTick(IExplosiveEntity entity) {
		...
		if(!level.isClientSide && entity.getTNTFuse() < 100) {
			if(entity.getPersistentData().getInt("nextExplosion") == 0) {
				...
				level.playSound((Entity)entity, toBlockPos(entity.getPos()).add(toBlockPos(randomPos)), SoundEvents.GENERIC_EXPLODE.value(), SoundSource.BLOCKS, 4f, (1f + (level.random.nextFloat() - level.random.nextFloat()) * 0.2f) * 0.7f);
