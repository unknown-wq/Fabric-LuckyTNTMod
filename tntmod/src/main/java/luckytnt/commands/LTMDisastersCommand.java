...
public class LTMDisastersCommand {
	...
	public static int executeGetActiveDisasters(CommandContext<CommandSourceStack> command) {
		if(command.getSource().getEntity() instanceof Player) {
			...
			if(LevelVariables.get(level).doomsdayTime > 0) {
				command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.doomsdayactive").append(Component.literal("" + LevelVariables.get(level).doomsdayTime / 1200)).append(Component.translatable("command.ltmdisaster.minute")), false);
				disasterActive = true;
			}
			if(LevelVariables.get(level).toxicCloudsTime > 0) {
				command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.toxicactive").append(Component.literal("" + LevelVariables.get(level).toxicCloudsTime / 1200)).append(Component.translatable("command.ltmdisaster.minute")), false);
				disasterActive = true;
			}
			if(LevelVariables.get(level).iceAgeTime > 0) {
				command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.iceageactive").append(Component.literal("" + LevelVariables.get(level).iceAgeTime / 1200)).append(Component.translatable("command.ltmdisaster.minute")), false);
				disasterActive = true;
			}
			if(LevelVariables.get(level).heatDeathTime > 0) {
				command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.heatdeathactive").append(Component.literal("" + LevelVariables.get(level).heatDeathTime / 1200)).append(Component.translatable("command.ltmdisaster.minute")), false);
				disasterActive = true;
			}
			if(LevelVariables.get(level).tntRainTime > 0) {
				command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.tntrainactive").append(Component.literal("" + LevelVariables.get(level).tntRainTime / 1200)).append(Component.translatable("command.ltmdisaster.minute")), false);
				disasterActive = true;
			}
			if(!disasterActive) {
				command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.nothingactive"), false);				
			}
		}
		return 1;
	}
	...
	public static int executeClear(CommandContext<CommandSourceStack> command) {
		if(command.getSource().getEntity() instanceof Player) {
			...
			command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.clear"), false);
		}		
		return 1;
	}
	...
	public static int executeDoomsday(CommandContext<CommandSourceStack> command) {
		if(command.getSource().getEntity() instanceof Player) {
			...
			command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.doomsday"), false);
		}		
		return 1;
	}
	...
	public static int executeToxicClouds(CommandContext<CommandSourceStack> command) {
		if(command.getSource().getEntity() instanceof Player) {
			...
			LevelVariables.get(level).sync(level);
			command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.toxicclouds"), false);
		}		
		return 1;
	}
	...
	public static int executeIceAge(CommandContext<CommandSourceStack> command) {
		if(command.getSource().getEntity() instanceof Player) {
			...
			command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.iceage"), false);
		}		
		return 1;
	}
	...
	public static int executeHeatDeath(CommandContext<CommandSourceStack> command) {
		if(command.getSource().getEntity() instanceof Player) {
			...
			LevelVariables.get(level).sync(level);
			command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.heatdeath"), false);
		}		
		return 1;
	}
	...
	public static int executeTNTRain(CommandContext<CommandSourceStack> command) {
		if(command.getSource().getEntity() instanceof Player) {
			...
			LevelVariables.get(level).sync(level);
			command.getSource().sendFeedback(() -> Component.translatable("command.ltmdisaster.tntrain"), false);
		}		
		return 1;
	}
}
