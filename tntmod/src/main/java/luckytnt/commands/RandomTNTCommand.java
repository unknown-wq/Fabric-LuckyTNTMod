	...
	public static int executeGiveItems(CommandSourceStack command, int amount, boolean allowDuplicate, String key) {
		if(command.getEntity() instanceof Player player) {
			if(allowDuplicate) {
				...
				int l = j;
				command.sendFeedback(() -> Component.translatable("command.randomtnt.success1").append(Component.literal(Integer.toString(l))).append(Component.translatable("command.randomtnt.success2")), false);
			} else {
				int j = 0;
				int tries = 0;
